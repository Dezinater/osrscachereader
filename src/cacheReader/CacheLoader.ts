...
export default class CacheLoader {
    ...
    private indexFiles = new Array(21).fill(0).map((_, i) => "main_file_cache.idx" + i).concat("main_file_cache.idx255");
    ...
    fetchURL(url: string) {
        ...
        this.promises.datFile = axios.get(url + this.datFile, { onDownloadProgress: this.onDownloadProgress, responseType: 'arraybuffer', });
        this.indexFiles.forEach(indexFile => {
            this.promises.indexFiles.push(axios.get(url + indexFile));
        });
    }
